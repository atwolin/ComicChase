...
from comic.models import Publisher, Comic, VolumeJp, VolumeTw
...
class ComicScrapersPipeline:
    ...
    def _get_book_title_tw(self, book_title: str):
        ...
        # Update title_tw
        series_name = ' '.join(parts[:-1]).strip()

        # Update latest_volume_tw if is final volume
        if is_final_volume:
            latest_volume_tw = volume_number

        return series_name, volume_number, is_final_volume, latest_volume_tw
    ...
    def _process_orphan_map_item(self, item: OrphanMapItem, spider):
        ...
        series_name, volume_number, is_final_volume, latest_volume_tw = self._get_book_title_tw(
        ...
        # 3. Update VolumeTw
        volume = VolumeTw.objects.filter(isbn_tw=isbn_tw).first()
        if volume:
            volume.comic = comic
            volume.title_tw = adapter.get('title_tw').strip()
            volume.volume_number = volume_number
            volume.release_date_tw = release_date_tw
            volume.publisher_tw = publisher
            volume.save()
            spider.logger.info(f"Updated VolumeTw: {volume}")
        else:
            spider.logger.warning(f"VolumeTw with ISBN {isbn_tw} not found to update.")
    ...
    def _get_book_description_jp(self, product_desc: str):
        ...
        Example formats:
            ...
            "発売日：2025年09月18日\n"
        """
        match = self.DATE_JP_REGEX.search(product_desc)
        if match:
    ...
    def _get_volume_number_jp(self, book_title: str):
        ...
        title_jp = book_title.translate(str.maketrans('０１２３４５６７８９', '0123456789')).strip()
        if matches:
    ...
    def _process_jp_comic_item(self, item: JpComicItem, spider):
        ...
        # 2. Find Comic by series_name
        comic = Comic.objects.filter(title_jp__in=[adapter.get('series_name')]).first()

        if not comic:
            raise DropItem(f"No matching Comic found for title_jp: {title_jp}")

        # Update author if available
        if author_jp_str:
            comic.author_jp = author_jp_str
            comic.save()
            spider.logger.info(f"Updated Comic author: {comic}")

        # 3. Update Volume
        volume = VolumeJp.objects.get_or_create(
            isbn_jp=isbn_jp,
            defaults={
                'comic': comic,
                'title_jp': title_jp,

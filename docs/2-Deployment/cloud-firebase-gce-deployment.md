  ...
  "rewrites": [
    {
      "source": "/api/**",
      "run": {
        "serviceId": "comicchase-service",  // ← Cloud Run service 名稱
        "region": "us-central1"
      }
    }
  ]
...
這個設定**不適用於 GCE**，需要修改才能讓 Firebase Hosting 指向 GCE backend。

---

#### **方案：同時支援 Cloud Run 和 GCE 兩個版本**

如果你想要同時保留兩種部署方式（gcr.py 和 gce.py），建議創建兩個 Firebase 配置檔案。

##### **步驟 1：創建 GCE 專用的 firebase.gce.json**

```bash
cd ui
```

創建新檔案 `firebase.gce.json`：
...
**重點差異：**
- ✅ `serviceId` 改為完整的 GCE URL（`https://api.comicchase.com.tw`）
- ✅ 新增 `/admin/**` 和 `/django-static/**` 的 rewrite 規則
- ✅ 移除 `region` 參數（不需要，因為 GCE 是固定 URL）

---

##### **步驟 2：配置 .firebaserc（多專案支援）**

**選項 A：使用同一個 Firebase 專案（簡單）**

保持現有的 `.firebaserc` 不變，部署時手動指定配置檔：
...
**選項 B：使用兩個 Firebase 專案（推薦）**

創建兩個不同的 Firebase 專案，分別用於 Cloud Run 和 GCE。

**1. 在 Firebase Console 創建第二個專案**

訪問 [Firebase Console](https://console.firebase.google.com)，創建新專案：
- 專案名稱：`comicchase-gce`（或其他名稱）

**2. 更新 `.firebaserc`**

編輯專案根目錄的 `.firebaserc`（不是 `ui/.firebaserc`）：

```json
{
  "projects": {
    "default": "comicchase-gcr",    // Cloud Run 版本
    "gce": "comicchase-gce"          // GCE 版本（新增）
  }
}
```

**3. 初始化 GCE 專案的 Firebase Hosting**
...
# 選擇 ui/dist 作為 public directory
```

**4. 部署到不同專案**

```bash
# 部署 Cloud Run 版本到預設專案
firebase use default
firebase deploy --only hosting

# 部署 GCE 版本到 GCE 專案
firebase use gce
firebase deploy --only hosting --config ui/firebase.gce.json
```
...
| 項目 | Cloud Run (firebase.json) | GCE (firebase.gce.json) |
|------|--------------------------|------------------------|
| **serviceId** | `comicchase-service`（Cloud Run service 名稱） | `https://api.comicchase.com.tw`（完整 URL） |
| **region** | ✅ 需要（`us-central1`） | ❌ 不需要 |
| **/api/ rewrite** | ✅ 有 | ✅ 有 |
| **/admin/ rewrite** | ❌ 沒有（Cloud Run 會自動處理） | ✅ **必須有** |
| **/django-static/ rewrite** | ❌ 沒有 | ✅ **必須有** |
| **Firebase URL** | `https://comicchase-gcr.web.app` | `https://comicchase-gce.web.app` |

---

##### **步驟 4：快速切換指令（使用別名）**

可以在 `package.json` 或創建 shell 腳本來簡化部署：
...
```json
{
  "scripts": {
    "build": "vite build",
    "deploy:gcr": "npm run build && firebase use default && firebase deploy --only hosting",
    "deploy:gce": "npm run build && firebase use gce && firebase deploy --only hosting --config firebase.gce.json"
  }
}
```
...
**Cloud Run 版本：**
1. 訪問 `https://comicchase-gcr.web.app/api/comics/series/`
2. 確認請求被轉發到 Cloud Run

**GCE 版本：**
1. 訪問 `https://comicchase-gce.web.app/api/comics/series/`
2. 確認請求被轉發到 GCE
3. 測試 Admin：`https://comicchase-gce.web.app/admin/`

---

#### **如果只使用 GCE（不需要 Cloud Run）**

如果你決定只使用 GCE，可以直接修改現有的 `firebase.json`：

```bash
cd ui
```

修改 `firebase.json` 為 GCE 版本（參考上面的 `firebase.gce.json` 內容）。
...
| 場景 | 做法 |
|------|------|
| **只用 Cloud Run** | 保持現有 `firebase.json` 不變 |
| **只用 GCE** | 修改 `firebase.json` 為 GCE 版本 |
| **同時支援兩者** | 創建 `firebase.gce.json` + 配置 `.firebaserc` |
